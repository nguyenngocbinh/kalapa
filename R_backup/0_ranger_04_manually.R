...
print(filter)
filter_imp <- filter$calculate(imputer_task)
fs <- head(as.data.table(filter_imp), 20)
fs

fs_task = imputer_task
fs_task$col_roles$feature <- fs$feature

opb$train(list(fs_task))
imb_task = opb$predict(list(fs_task))[[1]]

learner$train(imb_task)


learner$predict(imb_task, row_ids = train_idx)$confusion
learner$predict(fs_task, row_ids = test_idx) %>% as.data.table() %>% pull(response) %>% table()

x <- fs_task$backend$data(rows = 1:50000, cols = fs_task$feature_names)
# =============================================================================
## 4.2. Make graph
...
# store data
save(glrn, instance, task, test_idx, train_idx, file = "results/folder_ranger/ranger_04.Rdata")
...
glrn$predict(task, row_ids = test_idx) %>%
  ...
  mutate(id = id - 1) %>%
  rio::export("results/folder_ranger/ranger_04.csv")

# Tai lap ket qua
load("results/folder_ranger/ranger_04.Rdata")
